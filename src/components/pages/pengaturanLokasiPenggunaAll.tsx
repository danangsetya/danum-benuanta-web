...
import { lokasiT } from "@/lib/types";
import PengaturanLokasiPenggunaForm from "./pengaturanLokasiPenggunaForm";
type levelT = {
  id?: number;
  name: string;
  description: string;
...
const useSearch = (param: string, page: number) => {
  ...
  const [data, setData] = useState<lokasiT[]>([]);
  ...
  const getData = async (cPage = 0) => {
    return new Promise<resD>((resolve, reject) => {
      setLoading(true);
      fetch("/api/pengaturan/pengguna/lokasi/list", {
        method: "POST",
        headers: {
...
export default function PengaturanLokasiPenggunaAll() {
  const [subState, setSub] = useState<
    "new" | "all" | "edit_akses" | "edit_name"
  >("all");
  const [param, setParam] = useState("");
  const [page, setPage] = useState(1);
  const [detail, setDetail] = useState<lokasiT>();
  ...
  return (
    <>
      ...
      <h1 className="text-center font-bold text-xl">Semua Lokasi Absen</h1>
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead>Latittude</TableHead>
            <TableHead>Longtittude</TableHead>
            <TableHead></TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                          onClick={() => {
                            // console.log("click");
                            fetch(
                              "/api/pengaturan/pengguna/lokasi/delete?id=" +
                                item.id,
                              {
                                method: "GET",
                                headers: {
