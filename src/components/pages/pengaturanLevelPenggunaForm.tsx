...
export default function PengaturanLevelPenggunaForm({
  pNama = "",
  pDes = "",
  pId = 0,
  callback = () => {},
  ...
  const handleTambah = () => {
    ...
    if (nama == "" || desk == "") {
      ...
      setLoading(true);
      if (pId !== 0 && pNama !== "" && pDes !== "") {
        fetch("/api/pengaturan/pengguna/level/update", {
          method: "POST",
        ...
        fetch("/api/pengaturan/pengguna/level/add", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
